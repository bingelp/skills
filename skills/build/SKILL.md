...
A worktree-isolated session's `Edit`/`Write` tools may refuse this path since it sits outside the worktree root. That's expected, not a blocker: fall back to `Bash` (e.g. `python3` or a heredoc) to update `tasks.md` and append task notes when this happens. This path is deliberately untracked and shared across every worktree, so writing to it doesn't touch anything the isolation guard is protecting — it's the one sanctioned exception to the Red Flag below about routing around blocked writes, and it does not extend to source files.
...
4. Instruct the subagent to implement just that task, verify it (run it, run relevant tests, check the behavior), check it off in `tasks.md`, and append a **task note** to that task's line: what changed, any decisions or deviations from the plan, and verification evidence (falling back to `Bash` for the `tasks.md` write itself if `Edit`/`Write` refuses it — see "Where artifacts live"). If it hits something the plan didn't anticipate — including a tool refusing to edit a **source file** (e.g. a worktree-isolation guard blocking `Edit`/`Write` on a tracked file) — it should stop without working around the refusal or reconciling `plan.md`/`spec.md` itself, and report the blocker back instead.
