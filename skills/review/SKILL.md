...
2. Diff the actual implementation against `specs/<slug>/plan.md`'s stated approach. Note any deviations and whether they were justified — `specs/<slug>/tasks/NN-slug.md` (each task's note, pointed to from `tasks.md`) often already records a deviation the subagent flagged during `/build`; check there before assuming a mismatch is undocumented.
